...
class MemberController extends Controller
{
    public function dashboard()
    {
        ...
        $stats = [
            'aktif'    => $user->peminjaman->whereIn('status',['borrowed','overdue','return_pending'])->count(),
    ...
    {
        // Backwards compatibility: user no longer directly confirms return.
        // Keamanan: jika user mencoba akses route lama, ubah menjadi request pengembalian.
        return $this->requestKembalikan($peminjaman);
    }

    public function requestKembalikan(Peminjaman $peminjaman)
    {
        if ($peminjaman->id_user !== Auth::id()) abort(403);
        if (!in_array($peminjaman->status, ['borrowed','overdue'])) {
            return back()->with('error', 'Status tidak valid untuk request pengembalian.');
        }

        // Jika sudah pernah di-request, cegah duplikasi
        if ($peminjaman->status === 'return_pending') {
            return back()->with('error', 'Request pengembalian sudah diajukan dan menunggu konfirmasi admin.');
        }
        ...
        // Notifikasi admin (semua admin/petugas)
        \App\Models\User::where('level','admin')->orWhere('level','petugas')
            ->each(fn($admin) => \App\Models\Notifikasi::create([
                'id_user' => $admin->id_user,
                'judul'   => 'Request Pengembalian',
                'pesan'   => "{$peminjaman->nama_peminjam} meminta konfirmasi pengembalian buku \"{$peminjaman->buku->judul}\".",
                'tipe'    => 'warning',
                'url'     => route('peminjaman.index') . '?status=return_pending',
            ]));

        return back()->with('success', 'Request pengembalian berhasil dikirim. Menunggu konfirmasi admin.');
    }
