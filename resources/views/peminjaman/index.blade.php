                ...
                <tr>
                    ...
                    <td>
                        <div style="display:flex;gap:10px;align-items:flex-start;">
                            <div style="width:34px;height:46px;border-radius:6px;overflow:hidden;background:rgba(255,255,255,.06);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                                @if($p->buku?->cover)
                            ...
                            <div style="min-width:0;">
                                <div style="font-size:13px;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ Str::limit($p->buku?->judul,28) }}</div>
                    ...
                    <td style="font-size:12px;">
                        @if($p->due_date)
                            ...
                            {{-- Jika user mengembalikan sebelum jatuh tempo, tampilkan tanggal dikembalikan --}}
                            @if($p->return_date && !$p->isOverdue())
                                <br><span class="badge badge-success" style="display:inline-flex;align-items:center;gap:6px;">
                                    <i class="fas fa-check" style="font-size:11px"></i>
                                    Dikembalikan: {{ $p->return_date->format('d M Y') }}
                                </span>
                            @endif
                    ...
                    <td>
                        <div style="display:flex;gap:5px;flex-wrap:wrap;">
                            ...
                            {{-- KONFIRMASI PENGEMBALIAN (return_pending) --}}
                            @if($p->status === 'return_pending')
                            <button onclick="openModal('konfirmasi-kembali-{{ $p->id_pinjam }}')" class="btn btn-sm btn-warning btn-icon" title="Konfirmasi Pengembalian"><i class="fas fa-check-double"></i></button>
                            @endif
                        ...
                        {{-- Modal Kembalikan (langsung untuk borrowed/overdue) --}}
                        ...
                        {{-- Modal Konfirmasi Return Pending --}}
                        @if($p->status === 'return_pending')
                        <div class="modal-overlay" id="konfirmasi-kembali-{{ $p->id_pinjam }}">
                            <div class="modal" style="max-width:400px;">
                                <div class="modal-header">
                                    <h3>Konfirmasi Pengembalian</h3>
                                    <button class="modal-close" onclick="closeModal('konfirmasi-kembali-{{ $p->id_pinjam }}')"><i class="fas fa-times"></i></button>
                                </div>
                                <p style="font-size:13px;color:var(--text2)">
                                    Konfirmasi request pengembalian dari <strong>{{ $p->nama_peminjam }}</strong> untuk buku
                                    <strong>{{ $p->buku?->judul }}</strong>?
                                </p>
                                <div style="font-size:12px;color:var(--text2);margin-top:8px;">
                                    Status saat ini: <span class="badge badge-warning">{{ $p->statusLabel() }}</span>
                                </div>
                                <div class="form-group" style="margin-top:14px;">
                                    <label class="form-label">Catatan (opsional)</label>
                                    <textarea name="catatan_admin" form="form-konfirmasi-{{ $p->id_pinjam }}" class="form-control" rows="2" placeholder="Catatan untuk peminjam..."></textarea>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" onclick="closeModal('konfirmasi-kembali-{{ $p->id_pinjam }}')" class="btn btn-secondary btn-sm">Batal</button>
                                    <form method="POST" action="{{ route('peminjaman.konfirmasiKembalikan',$p->id_pinjam) }}" id="form-konfirmasi-{{ $p->id_pinjam }}">
                                        @csrf @method('PATCH')
